...
/**
 * The mosque's public identity.
 *
 * One form in two presentations rather than two components. In read mode the values are a `<dl>`; in
 * edit mode the same values are inputs. Keeping it one component is what guarantees the read view can
 * never show a field the edit view forgot — a mismatch that is very easy to introduce and very hard to
 * notice.
 *
 * Front-end only: `save` copies the draft into state and raises a toast. `cancel` throws the draft
 * away and restores what was showing. Nothing is posted.
 */
...
export function MosqueProfileView() {
  ...
  const save = () => {
    ...
    notify({
      ...
      description: "Changes are held in the browser — the mosque record is not connected yet.",
    });
  };
    ...
    <div className="space-y-4">
      ...
      {editing ? (
        <InlineNotice tone="gold" icon="pencil">
          You are editing the mosque profile. Nothing is saved until you choose <strong>Save Changes</strong> — and
          even then it stays in this browser, because the mosque record is not connected yet.
        </InlineNotice>
      ) : null}
      ...
      {editing ? (
        <Panel>
          <PanelFooter className="justify-end">
            ...
            <Button icon="check" disabled={nameMissing} onClick={save}>
              Save Changes
            </Button>
          </PanelFooter>
        </Panel>
 ...
 * Stand-in for a map.
 *
 * Deliberately not a map: no tile provider is wired up, and a grey rectangle that looks like a map but
 * cannot be panned is worse than one that says what it is. The address underneath is the part that
 * actually helps someone find the mosque, so that is the real content here.
 */
