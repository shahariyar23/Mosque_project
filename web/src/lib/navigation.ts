    ...
    items: [
      {
        ...
        icon: "book",
        permission: "quran.view",
      },
      {
        label: "Khutbah",
        labelBn: "খুতবা",
        href: "/dashboard/khutbah",
        icon: "megaphone",
        permission: "khutbah.view",
      },
      {
        label: "Articles",
        labelBn: "প্রবন্ধ",
...
export function filterNavigation(granted: Permission[]): ResolvedNavGroup[] {
  ...
  for (const group of NAVIGATION) {
    ...
    for (const item of group.items) {
      ...
      const own = item.ownPermission
        ? granted.includes(item.ownPermission)
        : false;
