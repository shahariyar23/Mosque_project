export interface PasswordSuccessTemplateData {
  loginUrl: string;
  userName?: string;
  mosqueName?: string;
...
/**
 * Renders the 04-password-success.html template preserving the NOOR design system.
 */
export function renderPasswordSuccessHtml(data: PasswordSuccessTemplateData): string {
...
<body width="100%" style="margin: 0; padding: 0 !important; mso-line-height-rule: exactly; background-color: #f8f6ef;">
  <center style="width: 100%; background-color: #f8f6ef; pb-8">
    ...
    <div style="text-align: center; margin-bottom: 32px;">
      <a href="${data.loginUrl}" class="btn btn-emerald">Sign In Now</a>
    </div>
...
export function renderPasswordSuccessText(data: PasswordSuccessTemplateData): string {
  const support = data.supportEmail || 'info@mostak.tech';
  const mosque = data.mosqueName || 'Noor Community Mosque';

  return `Password Updated

Your password has been successfully changed. You can now use your new password to access your account at:
