...
/**
 * Renders the 03-forgot-password.html template preserving the NOOR design system.
 */
export function renderForgotPasswordHtml(data: ForgotPasswordTemplateData): string {
  ...
  const expiry = data.expiresIn || '30 minutes';
...
<body width="100%" style="margin: 0; padding: 0 !important; mso-line-height-rule: exactly; background-color: #f8f6ef;">
  <center style="width: 100%; background-color: #f8f6ef; pb-8">
    ...
    <div style="text-align: center; margin-bottom: 24px;">
      <span style="font-size: 40px; color: #073a2d; line-height: 1;">🔐</span>
    </div>
    ...
    <p class="body-text" style="margin: 0 0 16px 0; text-align: center; font-size: 13px;">This link is valid for ${expiry}.</p>
...
export function renderForgotPasswordText(data: ForgotPasswordTemplateData): string {
  const expiry = data.expiresIn || '30 minutes';
  const support = data.supportEmail || 'info@mostak.tech';
...
We received a request to reset the password for your NOOR account. You can set a new password by visiting the link below:
